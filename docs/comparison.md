...
`tracelens` is one of several tools for LangChain observability. Each has a
sweet spot. Here's when to pick which.

## Decision tree

- **You want zero infrastructure (no Docker, no Postgres, no cloud signup)**
  and a UI you can open immediately → **tracelens**
...
| Feature | tracelens | LangSmith | LangFuse | Phoenix |
|---|---|---|---|---|
...
`tracelens` aims to be the **smallest possible thing** that gives you a useful
...
to debug a LangGraph at 2am on a flight with no internet, `tracelens` works.

## Migration / coexistence

The data directory is portable JSONL-exportable. You can:

- Run `tracelens` in dev, switch to LangSmith in production.
- Run both — multiple callbacks work simultaneously: `callbacks=[tracer.handler, langsmith_tracer]`.
- Export to JSONL via `tracelens export --all` and import elsewhere.

## When NOT to use tracelens
