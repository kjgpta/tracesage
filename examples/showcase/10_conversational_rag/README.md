...
chat history into a standalone search query (so "how much does it cost?" becomes "TraceLens
...
The only difference is `from tracelens import TraceLens`, wrapping the turn loop in
`async with TraceLens.session(install=True)`, and one `await tl.flush()` (plus a keep-the-UI-up
