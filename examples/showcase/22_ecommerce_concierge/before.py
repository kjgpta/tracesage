"""22 — E-commerce Shopping Concierge (plain LangChain).

An AgentExecutor that helps a shopper find and buy products. It has three local action
tools: search_catalog (canned product list), add_to_cart (mutates an in-memory cart),
and view_cart. The agent decides which tools to call and in what order. Pattern:
tool-calling agent driving side-effecting action tools.
