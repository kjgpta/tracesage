"""27 — Map-Reduce Long-Doc Summarizer (plain LangChain).

Splits a long document into ~5 chunks, MAPs a summarize chain over every chunk in
parallel (chain.batch), then REDUCEs the chunk summaries into one final summary.
Pattern: map-reduce fan-out — the classic shape for summarizing text that does not
