"""26 — Eval regression test using the `tracesage_capture` pytest fixture.

The fixture (registered automatically by the tracesage pytest11 entry point) installs
...
async def test_eval_batch_is_clean_and_cheap(tracesage_capture) -> None:
    graph = build_graph()
    for item in DATASET:
        await graph.ainvoke(
    ...
    tracesage_capture.assert_run_count(len(DATASET))

    tok_in, tok_out = tracesage_capture.total_tokens()
