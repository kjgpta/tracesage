"""26 — LLM-as-Judge Eval Harness (plain LangGraph).

Runs a tiny eval dataset (3 question/expected pairs) through a two-node LangGraph:
a *task* node answers the question, then a *judge* node scores correctness 0.0-1.0
with a one-line rationale (structured output). The batch loops over the dataset and
prints a score table — the classic offline-eval shape used in ML ops.
