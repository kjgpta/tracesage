...
pip install 'tracesage[mcp]' mcp langchain-mcp-adapters   # MCP extras (guarded in code)
...
The only differences are `from tracesage import TraceSage`, the
`from tracesage.adapters.mcp import register_mcp_client` helper, wrapping the run in
`async with TraceSage.session(install=True)`, swapping `client.get_tools()` for
