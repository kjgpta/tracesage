"""30 — Insurance Claim Intake & Routing (plain LangGraph).

A regulated-domain intake pipeline. Nodes: extract (pull structured claim fields via
with_structured_output) → validate (completeness check + a simple fraud-signal heuristic)
→ a conditional edge that routes to auto_approve / manual_review / fraud_review.
Pattern: extract-validate-route, the classic deterministic-rails-around-an-LLM shape.
