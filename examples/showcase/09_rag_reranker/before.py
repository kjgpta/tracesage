"""09 — RAG + Reranker (plain LangChain).

Retrieves top-8 chunks from a small local Chroma store, then runs an LLM-as-reranker
step that scores and reorders them down to top-3, and finally answers with inline
bracket citations like [1]. Pattern: retrieve-rerank-cite. A real cross-encoder (e.g.
a BAAI/bge reranker) could swap straight into the rerank step in place of the LLM scorer.
