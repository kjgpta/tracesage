"""Tests for TraceLensConfig validation (production-safety rail + numeric caps)."""
...
def test_non_loopback_without_token_fails_stop(host: str) -> None:
    ...
    with pytest.raises(ValidationError, match="TRACELENS_AUTH_TOKEN"):
        TraceLensConfig(host=host)


@pytest.mark.parametrize("host", ["127.0.0.1", "localhost", "::1"])
def test_loopback_hosts_need_no_token(host: str, tmp_path) -> None:
    cfg = TraceLensConfig(host=host, data_dir=tmp_path)
    assert cfg.host == host


def test_non_loopback_with_token_ok(tmp_path) -> None:
    cfg = TraceLensConfig(host="0.0.0.0", auth_token="secret", data_dir=tmp_path)  # noqa: S104
...
def test_ephemeral_port_zero_is_allowed(tmp_path) -> None:
    ...
    assert TraceLensConfig(port=0, data_dir=tmp_path).port == 0
...
def test_invalid_numeric_caps_rejected(kwargs: dict, needle: str, tmp_path) -> None:
    with pytest.raises(ValidationError, match=needle):
        TraceLensConfig(data_dir=tmp_path, **kwargs)
